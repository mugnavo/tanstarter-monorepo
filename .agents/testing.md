...
- Use a local integration test when multiple owned modules or a real disposable local dependency must cooperate. Isolate its data and never use development or production databases.
- Use Playwright for a small number of critical user journeys that unit tests cannot validate, such as auth entry points or payment completion.
